...
acc_b[["RNASeq2GeneNorm"]]@assays@data[[1]][1:5,1:3]
```

The data in the assay is a `DelayedMatrix` object:

```{r}
class(acc_b[["RNASeq2GeneNorm"]]@assays@data[[1]])
```

This is in contrast to the `acc` object that has matrices in memory:

```{r}
acc[["RNASeq2GeneNorm"]]@assays@data[[1]][1:5,1:3]
print(class(acc[["RNASeq2GeneNorm"]]@assays@data[[1]]))
```
