...
assays(acc_b[["RNASeq2GeneNorm"]])[[1]][1:5,1:3]
```

The data in the assay is a `DelayedMatrix` object:

```{r}
class(assays(acc_b[["RNASeq2GeneNorm"]])[[1]])
```

This is in contrast to the `acc` object that has matrices in memory:

```{r}
assays(acc[["RNASeq2GeneNorm"]])[[1]][1:5,1:3]
print(class(assays(acc[["RNASeq2GeneNorm"]])[[1]]))
```
