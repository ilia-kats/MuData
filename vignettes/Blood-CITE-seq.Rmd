...
We will use CITE-seq data available within [`CiteFuse` Bioconductor package](http://www.bioconductor.org/packages/release/bioc/html/CiteFuse.html).
...
```{r}
sce_citeseq <- CiteFuse::normaliseExprs(sce_citeseq, altExp_name = "ADT", transform = "log")
...
An appropriate structure for multimodal datasets is [`MultiAssayExperiment`](https://bioconductor.org/packages/MultiAssayExperiment/).
